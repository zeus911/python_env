...
class FileSystem:
    ...
    @property
    def status(self):
        if self.device.path in syblock.detec_procMounts():
            return True
        else:
            return False
...
class ReiserFileSystem(FileSystem):
    ...
    def format(self, partition):
        ...
        p.write("y\n")
        if p.close():
            raise rror, "reiserfs format failed: %s" % partition.path
...
class SwapFileSystem(FileSystem):
    ...
    def format(self, partition):
        ...
        res = sysutils.run(cmd)
        if not res:
            raise rror, "Swap format failed: %s" % partition.path
...
class NTFSFileSystem(FileSystem):
    ...
    def format(self, partition):
        ...
        res = sysutils.run(cmd)
        if not res:
            raise rror, "Ntfs format failed: %s" % partition.path
...
class FatFileSystem(FileSystem):
    ...
    def format(self, partition):
        ...
        res = sysutils.run(cmd)
        if not res:
            raise rror, "vfat format failed: %s" % partition.path
